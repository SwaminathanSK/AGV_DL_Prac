...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        out_relu1, relu_cache1 = relu_forward(out1)
        out2, cache2 = affine_forward(out_relu1, self.params['W2'], self.params['b2'])
        ...
        #L2 regularization. We should not consider b1, b2.

        dx2, grads['W2'], grads['b2'] = affine_backward(dout2,cache2)
        grads['W2'] += self.reg*self.params['W2'] # L2 Regularization
        dout1 = np.dot(dout2, self.params['W2'].T).reshape(out1.shape) # d(out2 = affine)/dout1
        # dout1 = (out1>0)*np.dot(dout2, self.params['W2'].T).reshape(out1.shape) # d(ou2 = relu + affine)/dout1

        dx1, grads['W1'], grads['b1'] = affine_relu_backward(dout1,(cache1, relu_cache1))
